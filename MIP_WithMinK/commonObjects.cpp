...
{
	status = CPXaddrows (env, problem, 0, nrows, nzcntA, rhs, sense, rmatbegA, rmatindA, rmatvalA, NULL, rownames);
	if ( status )
	{
...
{
	status = CPXaddrows (env, problem, 0, 1, row.nzcnt, row.rhs, row.sense, row.rmatbeg, row.rmatind, row.rmatval, NULL, rownames);
	if ( status )
	{
...
void RowFormat::optimize(bool isLinearProgramming)
{
   ...
   runTime = double(stop - go) / CLOCKS_PER_SEC;
}
	...
	if(isMaster)
	{
		...
		for(int s=0; s<index.s; ++s)
			for(int i=0; i<index.i; ++i)
				for(int j=0; j<index.i; ++j)
				{
					stringstream ss;
					string str;
					ss << parameter.colnames[0] << s+1 << i+1 << j+1;
					str =ss.str();
					strcpy (colnames[iteratorII], str.c_str());
					++iteratorII;
				}
		...
		for(int s=0; s<index.s; ++s)
			for(int i=0; i<index.i; ++i)
			{
				stringstream ss;
				string str;
				ss << parameter.colnames[0] << s+1 << i+1 ;
				str =ss.str();
				strcpy (colnames[iteratorII], str.c_str());
				++iteratorII;
			}
		...
		for(int q=0; q<(index.q); ++q)
			for(int s=0; s<index.s; ++s)
				for(int i=0; i<index.i; ++i)
				{
					stringstream ss;
					string str;
					ss << parameter.colnames[0] << q << s+1 << i+1;
					str =ss.str();
					strcpy (colnames[iteratorII], str.c_str());
					++iteratorII;
				}
...
int IntegerPower(int64_t bNum, uint8_t power) //64 bit
 {
	if(power > 30) 
	{
	 	cout<<"over flow"<<endl;
...
void readInput(ifstream& inMYFILE, char* fileAddress, IndicesRec& index, searchParameter& masterParam, searchParameter& subParam)
{
	...
	inMYFILE>> index.s;		//S
	inMYFILE>> index.i;		//P
	inMYFILE>> masterParam.lowerK;   //
	inMYFILE>> masterParam.K;	index.d=masterParam.K;
	inMYFILE>> masterParam.R;	index.l= masterParam.R;
	inMYFILE>> masterParam.sigma;
...
void RowFormat::printX( ofstream& AllFILE , const searchParameter& masterParam, const IndicesRec& index, double runTime, const char* input)
{
	int cursor =0;
	double realX;
	...
	AllFILE<< "sigma "<<"	"<< "S "<<"	"<< "P "<<"	"<< "lowerK "<<"	"<< "K "<<"	"<< "R "<<"	"<<"Obj "<<"	"<<"time(sec) "<< "	"<< "input "<<"	"<< "x's "<<"	"<< "L's "<<"	"<< "U's "<<"	"<<endl;
	AllFILE<<std::fixed<< masterParam.sigma<<"	"<< index.s<<"	"<< index.i<<"	"<< masterParam.lowerK<<"	"<< masterParam.K<<"	"<< masterParam.R<<"	"<<Round(objval- adj) <<"	"<<runTime<< "	"<< input<<"	";
	for(int s=0; s<index.s; ++s)
	{
		for (int i=0; i<index.i; ++i)
		{
			...
			for (int j=0; j<index.i; ++j)
			{
				cursor = index.s * index.l * (index.d+1) + s * index.i * index.i + index.i * i + j;
				realX += (j+1)*x[cursor];
			}
			tempX[i]=realX;
		}
	...
	for(int s=0; s<index.s; ++s)
	{
		for (int i=0; i<index.i; ++i)
		{
			cursor =  index.s * index.l *( index.d+1) + (index.s * index.i * index.i) +2*(index.s * index.i * (index.i+1))  + s *index.i + i;
			tempL[i]=x[cursor];
		}
		for (int i=0; i<index.i; ++i)
			AllFILE<<tempL[i]<<"	";
	...
	for(int s=0; s<index.s; ++s)
	{
		for (int i=0; i<index.i; ++i)
		{
			cursor = index.s * index.l *( index.d+1) + (index.s * index.i * index.i) +2*(index.s * index.i * (index.i+1))  +1* index.s *index.i + s *index.i + i;
			tempU[i]=x[cursor];
		}
		for (int i=0; i<index.i; ++i)
			AllFILE<<tempU[i]<<"	";
